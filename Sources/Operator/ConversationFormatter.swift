...
enum ConversationFormatter {
    ...
    /// Messages with no text content and no tool calls are skipped.
    ...
    /// Media parts are noted with placeholders like `[+2 attachments]`.
    ///
    ...
    static func format(_ messages: some Collection<LLM.OpenAICompatibleAPI.ChatMessage>) -> String {
        ...
        for message in messages {
            ...
            if let toolCalls = message.tool_calls, !toolCalls.isEmpty, message.textContent == nil {
            ...
            guard let content = message.textContent else {
                continue
            }

            var line = "[\(label)]: \(content)"

            // Append media placeholder if present
            let mediaCount = message.content.filter(\.isMedia).count
            if mediaCount > 0 {
                let noun = mediaCount == 1 ? "attachment" : "attachments"
                line += " [+\(mediaCount) \(noun)]"
            }
